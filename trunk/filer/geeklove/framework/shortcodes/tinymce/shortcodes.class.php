...
class stag_shortcodes{
	...
	function formate_shortcode(){
		...
		if( isset( $stag_shortcodes ) && is_array( $stag_shortcodes ) )
		{
			...
			{
				//$this->append_output( "\n" . '<div id="_stag_preview" class="hidden">false</div>' );
			...
			{
				// prefix the fields names and ids with stag_
				$pkey = 'stag_' . $pkey;
				...
				switch( $param['type'] )
				{
					...
					case 'textarea' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<textarea rows="10" cols="30" name="' . $pkey . '" id="' . $pkey . '" class="stag-form-textarea stag-input">' . $param['std'] . '</textarea>' . "\n";
						$output .= $row_end;
